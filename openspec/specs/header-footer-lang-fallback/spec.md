# Spec: Header / Footer Lang Fallback

## Purpose

Defines how the header and footer blocks resolve their fragment paths when page metadata is absent or the metadata-driven fetch fails. A URL-derived fallback path is computed from raw site and language segments in `window.location.pathname`, enabling correct fragment loading across multi-site, multi-language deployments without requiring per-page metadata authoring.
