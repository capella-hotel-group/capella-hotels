...
import { r as getPublishBaseUrl } from "../../scripts/vendor/env-0ww78MbC.js";
...
function resolveAssetUrl(damPath) {
	...
	return `${getPublishBaseUrl()}${damPath}`;
}
...
async function fetchCFDetails(cfPath) {
	try {
		const url = `${getPublishBaseUrl()}/graphql/execute.json/capella-hotels/TabList;path=${cfPath}`;
		...
		if (!item) console.error(`[culturist-carousel] No tabDetailsByPath item returned for ${cfPath}`, data);
		return item;
	} catch (error) {
...
async function decorate(block) {
	...
	if (rows[0]?.querySelector("picture, img")) firstDataRowIndex = 1;
	...
	const itemRows = rows.slice(firstDataRowIndex + 2).filter((row) => {
		return [...row.querySelectorAll(":scope > div")].some((cell) => cell.textContent?.trim() !== "");
	});
	...
	itemRows.forEach((row, index) => {
		...
		optionBtn.addEventListener("click", async () => {
			...
			if (index !== currentTabIndex) await selectTab(index);
		});
		listItem.append(optionBtn);
		destinationList.append(listItem);
	});
	destinationBtn.addEventListener("click", () => {
		if (destinationList.hidden) openDestinationList();
		else closeDestinationList();
	});
	document.addEventListener("click", (event) => {
		if (!destinationWrapper.contains(event.target)) closeDestinationList();
	});
	document.addEventListener("keydown", (event) => {
		if (event.key === "Escape") closeDestinationList();
	});
