/*! v1.3.0 | h6260283b */
import { i as resolveDAMUrl } from "../../scripts/vendor/env-Bvky9kZQ.js";
...
function decorate(block) {
	...
	if (contentEl) {
		...
		const scrollToNext = () => {
			const nextSection = block.closest(".hero-banner-wrapper")?.nextElementSibling ?? block.closest("div")?.nextElementSibling;
		...
		content.innerHTML = [...contentEl.querySelectorAll("p")].map((p) => p.textContent?.trim() ?? "").filter(Boolean).join("<br>");
