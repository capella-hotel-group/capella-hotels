...
import { l as getMetadata } from "../../scripts/vendor/aem-DT0e_OxR.js";
...
if (!("ontouchstart" in window) && !navigator.maxTouchPoints) document.documentElement.classList.add("no-touch");
/**
...
function buildNavZone(navItems) {
	...
	navItems.forEach((srcItem, i) => {
		...
		if (srcItem.querySelector(":scope > ul")) {
			...
			const triggerEl = wrapper.querySelector(".header-nav-drop-trigger");
			if (triggerEl) triggerEl.dataset.navSide = side;
...
async function decorate(block) {
	...
	const topLevelItems = navContent ? [...navContent.querySelectorAll(":scope > ul")].flatMap((ul) => [...ul.querySelectorAll(":scope > li")]) : [];
	const langList = topLevelItems[0]?.querySelector("ul");
	const navItems = topLevelItems.slice(1);
	...
	const currentPath = window.location.pathname;
	const activeLangHref = [...langList.querySelectorAll("li a")].map((a) => a.getAttribute("href")).filter((href) => !!href).find((href) => currentPath.startsWith(href)) ?? `${getFragmentBasePath()}/`;
