 ...
 */
function buildLangZone(sourceList: Element): HTMLDivElement {
  ...
  document.addEventListener('click', (e) => {
    if (!zone.contains(e.target as Node)) closeLangDropdown(trigger, dropdown);
  });
 ...
 */
function buildNavZone(navItems: Element[]): HTMLElement {
  ...
  navItems.forEach((srcItem, i) => {
    ...
    if (srcItem.querySelector(':scope > ul')) {
      ...
      const triggerEl = wrapper.querySelector<HTMLElement>('.header-nav-drop-trigger');
 ...
 */
function buildMobilePanel(navItems: Element[], langList: Element): HTMLDivElement {
...
export default async function decorate(block: HTMLElement): Promise<void> {
  ...
  const ctaAnchor = sections[1]?.querySelector<HTMLAnchorElement>('a') ?? null;
