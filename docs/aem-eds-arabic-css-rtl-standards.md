...
Detect language and apply direction in `src/app/scripts.ts` (compiled to `scripts/scripts.js` — never hand-edit the generated file) before any block is decorated:
...
>
> **Note:** the snippet above is simplified for illustration. The real `applyDirection(lang)` in `src/app/scripts.ts` takes the BCP 47 tag already resolved by `getPageLang()` (which parses the URL path against `LANG_MAP`/`VALID_LANG_PRIMARIES`, not `document.documentElement.lang` or a `meta[name="lang"]` tag) and is called as `applyDirection(getPageLang())` from `loadEager()`.
...
**Global overrides** go in `src/styles/styles.css`:
...
> **⚠️ Not yet implemented:** as of this writing, `src/styles/styles.css` does not load Cairo/Noto Sans Arabic or override `--body-font-family`/`--heading-font-family` for `[dir="rtl"]` — only `line-height`, `letter-spacing`, `word-spacing`, and the icon-flip rules from §1.5 are wired up. Treat the font setup below as the target standard, not the current state, until it's implemented.
...
```css
/* src/styles/styles.css — global icon flip rules */
...
```css
/* src/styles/styles.css — body typography for Arabic */
...
RTL detection in `src/app/scripts.ts` must cover all four language codes:
...
**RTL detection in `src/app/scripts.ts` — full tag support:**

> **Note:** simplified for illustration, same caveat as §1.1 — the real implementation resolves the tag via `getPageLang()`, not `document.documentElement.lang`/a meta tag.
...
- [ ] RTL detection in `src/app/scripts.ts` strips region suffix before checking language code
