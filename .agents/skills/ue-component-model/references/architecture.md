...
> **Adapted for this project:** the logical relationships below (definition → model → filter) are unchanged, but physically this repo authors them as one distributed `src/blocks/<name>/_<name>.json` per block (`definitions`/`models`/`filters` keys) plus shared fragments in `src/models/_*.json`, all merged by `npm run build:json` into the root `component-definition.json` / `component-models.json` / `component-filters.json`. Treat every `component-*.json` reference below as "the aggregated root file", never edited directly.

## The Three Files

An AEM Edge Delivery Services (aem.live) project using the Universal Editor (UE) requires three JSON configuration files, generated at the project root from source fragments in `src/models/` and `src/blocks/*/`:
...
5. Block JS decorates the HTML
   ├── blocks/<name>/<name>.js receives the block div (this project: compiled from src/blocks/<name>/<name>.ts)
