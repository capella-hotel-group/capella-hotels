...
> **Adapted for this project:** examples below use plain JavaScript with JSDoc types (the upstream convention), but this repo authors blocks in **TypeScript** under `src/blocks/<name>/<name>.ts`, compiled by Vite into the generated `blocks/<name>/<name>.js`. Translate each example's signature to `export default function decorate(block: HTMLElement): void { ... }` (or `Promise<void>` if async is needed) and drop the `@param`/`@returns` JSDoc in favor of native TS types. Use the `@/*` path alias for imports from `src/app/` or `src/utils/` instead of relative `../../scripts/aem.js` paths. The DOM patterns, performance techniques, and helper usage below are otherwise directly applicable.
