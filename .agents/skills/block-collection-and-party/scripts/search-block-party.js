...
function validateCategory(category, availableCategories) {
  ...
  const matchingCategory = availableCategories.find(
    (cat) => cat.toLowerCase() === lowerCategory || cat.toLowerCase().includes(lowerCategory),
  );
...
function searchEntries(entries, searchTerms) {
  ...
  return entries.filter((entry) => {
    const searchableText = [entry.title || '', entry.description || '', entry.category || '', entry.githubProfile || '']
      .join(' ')
      .toLowerCase();
...
async function main() {
  ...
  try {
    ...
    const approvedEntries = allEntries.filter(
      (entry) => entry.approved === 'Yes' || entry.approved === true || entry.approved === 'true',
    );
