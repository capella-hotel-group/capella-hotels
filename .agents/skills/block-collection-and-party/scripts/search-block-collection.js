...
function searchBlocks(blocks, searchTerm) {
  ...
  return blocks
    .filter((block) => {
      ...
      return (
        block.name.toLowerCase().includes(lowerSearchTerm) || block.displayName.toLowerCase().includes(lowerSearchTerm)
      );
    })
    .map((block) => {
      const result = {
      ...
      if (result.type === 'block') {
        ...
        result.note =
          'This is default content documentation, not a standalone block. Code may be part of other blocks (e.g., breadcrumbs are in the header block). Visit https://www.aem.live/developer/block-collection and the live example URL for implementation guidance.';
