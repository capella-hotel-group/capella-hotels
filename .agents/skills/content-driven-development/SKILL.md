...
description: Apply a Content Driven Development process to AEM Edge Delivery Services development in this project. Use for ALL code changes - new blocks, block modifications, CSS styling, bug fixes, core functionality (src/app/scripts.ts, styles, etc.), or any TypeScript/CSS work that needs validation.
license: Apache-2.0
metadata:
  version: '2.0.1-capella'
  adapted-from: 'adobe/skills content-driven-development v2.0.1 — dev server command and ship-it git paths adjusted for src/ + Vite/TypeScript pipeline'
---
...
- `npm start` runs `tsc --watch` + a Vite watch build of `src/` into `scripts/`, `styles/`, `blocks/*` **and** the AEM CLI dev server together — required so edits under `src/` are picked up. Do NOT run bare `aem up` (or `npx @adobe/aem-cli up`) as the primary dev command: it serves whatever is already in `blocks/`/`scripts/`/`styles/` without recompiling `src/`, so TypeScript/CSS edits won't show up.
- Run in background if possible (dev server needs to stay running)
- Only use `npx -y @adobe/aem-cli up --no-open --forward-browser-logs` directly if you truly only need the AEM CLI with no `src/` watch build — then you must manually run `npm run build:runtime` after every `src/` change.
...
2. **Rebuild, then stage both source and regenerated output:**

   ```bash
   npm run build:json   # if any _<block-name>.json model changed
   npm run build        # regenerate blocks/, scripts/, styles/ from src/
   git add src/blocks/{block-name}/{block-name}.ts src/blocks/{block-name}/{block-name}.css src/blocks/{block-name}/_{block-name}.json \
           blocks/{block-name}/{block-name}.js blocks/{block-name}/{block-name}.css
   # Add only the files you worked on - NEVER use `git add .`
   # Generated output (blocks/, scripts/, styles/, root component-*.json) IS committed
   # (AEM Code Sync serves directly from git) — but it must always be produced by
   # `npm run build`/`build:json` from src/, NEVER hand-edited.
   ```
