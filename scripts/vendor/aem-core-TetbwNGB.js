...
//#endregion
//#region src/app/aem.ts
var sampleRUM = (checkpoint, data) => {
	...
	try {
		...
		if (!window.hlx.rum || !window.hlx.rum.collector) {
			...
			const weight = rateValue !== void 0 ? rateValue : 100;
		...
		document.dispatchEvent(new CustomEvent("rum", { detail: {
			checkpoint,
			data
		} }));
	} catch (error) {}
...
function setup() {
	...
	if (scriptEl) try {
		...
		if (scriptURL.host === window.location.host) window.hlx.codeBasePath = scriptURL.pathname.split("/scripts/scripts.js")[0] ?? "";
		else window.hlx.codeBasePath = scriptURL.href.split("/scripts/scripts.js")[0] ?? "";
	} catch (error) {
		console.log(error);
	}
}
		...
		if (row.children) {
			...
			if (cols[1]) {
				...
				if (col.querySelector("a")) {
					...
					if (as.length === 1) value = as[0].href;
					else value = as.map((a) => a.href);
				} else if (col.querySelector("img")) {
					const imgs = [...col.querySelectorAll("img")];
					if (imgs.length === 1) value = imgs[0].src;
					else value = imgs.map((img) => img.src);
				} else if (col.querySelector("p")) {
					const ps = [...col.querySelectorAll("p")];
					if (ps.length === 1) value = ps[0].textContent;
					else value = ps.map((p) => p.textContent);
...
async function loadScript(src, attrs) {
	return new Promise((resolve, reject) => {
		if (!document.querySelector(`head > script[src="${src}"]`)) {
			const script = document.createElement("script");
			script.src = src;
			if (attrs) Object.entries(attrs).forEach(([attr, val]) => script.setAttribute(attr, val));
...
function wrapTextNodes(block) {
	...
	const wrap = (el) => {
		...
		[...el.attributes].filter(({ nodeName }) => nodeName === "class" || nodeName.startsWith("data-aue") || nodeName.startsWith("data-richtext")).forEach(({ nodeName, nodeValue }) => {
	...
	block.querySelectorAll(":scope > div > div").forEach((blockColumn) => {
		if (blockColumn.hasChildNodes()) {
			if (!(!!blockColumn.firstElementChild && validWrappers.some((tagName) => blockColumn.firstElementChild?.tagName === tagName))) wrap(blockColumn);
			else if (blockColumn.firstElementChild?.tagName === "PICTURE" && (blockColumn.children.length > 1 || !!blockColumn.textContent?.trim())) wrap(blockColumn);
		}
	});
}
/**
	...
	main.querySelectorAll(":scope > div:not([data-section-status])").forEach((section) => {
		...
		if (sectionMeta) {
			...
			Object.keys(meta).forEach((key) => {
				if (key === "style") String(meta.style).split(",").filter((style) => style).map((style) => toClassName(style.trim())).forEach((style) => section.classList.add(style));
				else section.dataset[toCamelCase(key)] = String(meta[key]);
			});
...
function buildBlock(blockName, content) {
	...
	table.forEach((row) => {
		...
		row.forEach((col) => {
			...
			(col && typeof col === "object" && "elems" in col ? col.elems : [col]).forEach((val) => {
				if (val) {
...
init();
//#endregion
export { wrapTextNodes as C, waitForFirstImage as S, readBlockConfig as _, decorateButtons as a, toCamelCase as b, decorateTemplateAndTheme as c, loadCSS as d, loadFooter as f, loadSections as g, loadSection as h, decorateBlocks as i, getMetadata as l, loadScript as m, createOptimizedPicture as n, decorateIcons as o, loadHeader as p, decorateBlock as r, decorateSections as s, buildBlock as t, loadBlock as u, sampleRUM as v, __vitePreload as w, toClassName as x, setup as y };
