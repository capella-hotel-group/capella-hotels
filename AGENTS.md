...
- TypeScript (ES6+) under `src/`, compiled with Vite/tsc into the plain-JS runtime AEM EDS expects — no framework, no transpiling at request time
...
- Regenerate the aggregated component JSON after editing block models: `npm run build:json`
...
├── docs/                        # Topic-specific developer guides
